(() => {
  ...
  const BATCH_SIZE = 25;
  const EXPORT_TIMESTAMP = "2026-06-24T00:00:00+00:00";
  ...
  function saveProgress() {
    ...
    progress[task.sample_id] = { order: currentOrder(), seconds: (progress[task.sample_id]?.seconds || 0) + elapsedSeconds(), updated_at: EXPORT_TIMESTAMP };
  ...
  function exportCsv() {
    saveProgress(); const progress = loadProgress(); const header = ["participant_id", "batch", "sample_id", "function_name", "rank", "candidate_label", "context_level", "seconds", "updated_at"];
    const rows = [header];
    state.tasks.forEach((task) => {
      ...
      record.order.forEach((level, index) => rows.push([state.participantId, state.batch, task.sample_id, task.function_name, index + 1, labelByLevel[level], level, record.seconds || 0, record.updated_at]));
    });
